...
AmazonMwsClient.prototype.parseXml = function(data, resolve, reject, action) {
	...
	xml2js.parseString(data, {
		...
		if (err != null) {
			...
			if (result.ErrorResponse && result.ErrorResponse.Error) {
				let err = result.ErrorResponse.Error[0];
				reject(new Error(err.Code[0] + ': ' + err.Message[0]));
			}

			result = this.formatResult(action, result);
			resolve(result);
		}
	}.bind(this));
};
...
/**
 * Provides a point to override for additional post processing
 * of the result object
 *
 * @param result
 * @returns {*}
 */
AmazonMwsClient.prototype.formatResult = function(action, result) {
	return result;
};
	...
	return new Promise(function(resolve, reject) {
		request(requestOpts, function(err, response, data) {
			if (err) {
				...
				if (data.slice(0, 5) == '<?xml' || data.indexOf('xmlns="') !== false) {
					this.parseXml(data, resolve, reject, action);
				} else {
					resolve(data);
				}
			}
